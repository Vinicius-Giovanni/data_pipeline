...
class BottleneckBoxPipeline:
    def __init__(self):
        self.pipeline_key = 'bottleneck_box'
    ...
    def run(self) -> pd.DataFrame:
        try:
            ...
            df_load = self.read_parquet_folder(
                self.paths['parquet_load'],
                self.cfg['read_columns_load']
            )
            ...
            if df_load.empty or df_putaway.empty:
                logger.warning('Dados de load ou putaway estão vazios.')
                return pd.DataFrame()
            ...
            df_all = pd.merge(df_load, df_putaway, on='olpn', how='left')
            ...
            df_all['diferenca_segundos'] = (
                (df_all['data_hora_load'] - df_all['data_hora_putaway'])
                .dt.total_seconds()
            ).astype('Int64')
            ...
            logger.error(f'Erro na execução do pipeline bottleneck_box: {e}')
            return pd.DataFrame()